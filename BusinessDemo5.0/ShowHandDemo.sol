...
contract ShowHandDemo {
    ...
    address payable private dealer; //平台方
    address payable public hostPlayer;  //主位
    address payable public guestPlayer; //客位
    address payable public activePlayer; //说话的玩家，即应该行动的玩家
    address payable public winner; //获胜者

    mapping(address => uint[5]) public pokers; //52张牌，uint8够用 0~255, 5代表5轮
    ...
    uint joinRand; //区分加入方为主方或客方,做个随机处理
    ...
    constructor() public{
        ...
        joinRand = uint(keccak256(abi.encodePacked(now,msg.sender,block.number))) % 2;
    }
    ...
    //加入游戏，主客随机而定
    function joinGame(address payable _joiner) public {
    ...
    event OnPokerDealLight(address player,uint poker);
    event OnPokerDealDark(address player,uint poker);
    ...
    //比较所有牌大小
    function comparePokers(uint _start,uint[5] memory _hostPokers,uint[5] memory _guestPokers) private pure returns (bool) {
    ...
    //查看暗牌
    function getPocket() public view returns (uint){
