...
contract DrawLots {
    ...
    function subgroup() public{
        ...
        if (keccak256(randArr[random]) == keccak256("A")) {
            groupA.push(teams[msg.sender].teamName);
            teams[msg.sender].inGroup = "groupA";
        } else if (keccak256(randArr[random]) == keccak256("B")) {
    ...
    //带view修饰符的直接在界面上可以看到
    function showGroupA() public view groupFull returns (string[]){
        return groupA;
    }
    function showGroupB() public view groupFull returns (string[]){
        return groupB;
    }
    function showGroupC() public groupFull returns (string[]){
        return groupC;
    }
